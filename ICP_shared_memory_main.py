...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s]  %(message)s", handlers=[logging.FileHandler("/home/hnguyen/LTM---Inter-Process-Communication/log/shared_memory.log", mode="w"), logging.StreamHandler()])
def producer(shm_name, shape, count, finished, cond, num_messages=5):
    shm = shared_memory.SharedMemory(name=shm_name)
    buf = np.ndarray(shape, dtype=np.int32, buffer=shm.buf)

    for i in range(num_messages):
        # data = np.random.randint(0, 100, size=shape, dtype=np.int32)
...
def consumer(shm_name, shape, count, finished, cond, cid=1):
    ...
    while True:
        ...
        logging.info(f"Consumer {cid} đọc message {last_seen}, mean={data_mean:.1f}")
...
def process_data(data):
    ...
    logging.info(f"Processing data with mean {data.mean()}")
    return data.mean()
